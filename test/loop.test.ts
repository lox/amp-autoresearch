...
test('kickoff prompts embed workdir, tool contract, and loop rules', async () => {
	...
	expect(create).toContain('NEVER STOP SILENTLY')
	expect(create).toContain('conclude_experiment')
	expect(create).toContain('Checkpoint pressure-test every ~5 keeps')
	...
	expect(resume).toContain('NEVER STOP SILENTLY')
	expect(resume).toContain('conclude_experiment')
})
...
test('final review message lists kept commits and forbids new experiments', async () => {
	...
	expect(msg).toContain('Unasserted invariants')
	expect(msg).toContain('Complexity vs win')
