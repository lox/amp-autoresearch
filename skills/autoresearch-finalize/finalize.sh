...
#
# Adapted for amp-autoresearch from pi-autoresearch (MIT,
# https://github.com/davebcn87/pi-autoresearch, skills/autoresearch-finalize).
# Changes: JSON parsing tries bun/node/python3 instead of requiring node.
#
...
parse_groups() {
  ...
  # Uses whichever JS/Python runtime is available — Amp environments guarantee
...
"
  if command -v bun >/dev/null 2>&1; then
    bun -e "$js_src" || fail "Failed to parse $groups_file — check JSON syntax."
  elif command -v node >/dev/null 2>&1; then
    node -e "$js_src" || fail "Failed to parse $groups_file — check JSON syntax."
  elif command -v python3 >/dev/null 2>&1; then
    python3 - "$groups_file" "$DATA_DIR" <<'PY' || fail "Failed to parse groups.json — check JSON syntax."
import json, sys
config = json.load(open(sys.argv[1]))
out = sys.argv[2]
def w(name, value):
    open(f"{out}/{name}", "w").write(str(value))
w("base", config["base"])
w("trunk", config.get("trunk", "main"))
w("final_tree", config["final_tree"])
w("goal", config["goal"])
w("count", len(config["groups"]))
for idx, group in enumerate(config["groups"]):
    for key in ("title", "body", "last_commit", "slug"):
        w(f"{idx}.{key}", group[key])
PY
  else
    fail "Need bun, node, or python3 to parse $groups_file."
  fi
